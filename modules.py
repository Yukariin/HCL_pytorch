...
def confidence_mask_single_stage(
    ...
    if last_stage:
        lowconf_mask = torch.ones_like(feature[:, :1, :, :]).bool()
        acc_valid_mask = torch.zeros_like(feature[:, :1, :, :]).bool()
        acc_invalid_mask = torch.zeros_like(feature[:, :1, :, :]).bool()
        acc_conf = torch.full_like(feature[:, :1, :, :], fill_value=0.5)
    else:
        lowconf_mask = ~(acc_valid_mask | acc_invalid_mask)
        lowconf_mask = upsample2x(lowconf_mask)
        acc_valid_mask = upsample2x(acc_valid_mask)
        acc_invalid_mask = upsample2x(acc_invalid_mask)
        acc_conf = upsample2x(acc_conf)
    cur_valid_mask = torch.zeros_like(feature[:, :1, :, :]).bool()
    cur_invalid_mask = torch.zeros_like(feature[:, :1, :, :]).bool()
    cur_conf = torch.full_like(feature[:, :1, :, :], fill_value=0.5)
    cur_centers = torch.zeros((bs, 2, C), device=feature.device)
    if gt_mask is not None:
        gt_mask = F.interpolate(gt_mask, size=(H, W))
    
    for b in range(bs):
        if (~lowconf_mask[b]).all():
            continue
        lowconf_feature = mask_indexing(feature[b], lowconf_mask[b])
        kmeans.fit(lowconf_feature)
        cur_conf[b] = kmeans.predict(feature[b].reshape(C, H*W).permute(1, 0)).reshape(1, H, W)
        cur_centers[b] = torch.clone(kmeans.centers)
